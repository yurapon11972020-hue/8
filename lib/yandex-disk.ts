...
function getSourceMode(): JournalSource {
  ...
  if (source === 'yandex-private' || source === 'yandex-public' || source === 'local') {
    return source;
  }
...
async function readFromPublicYandexDisk(): Promise<JournalFileResult> {
  const publicKey = process.env.YANDEX_DISK_PUBLIC_KEY?.trim();
  const publicPath = process.env.YANDEX_DISK_PUBLIC_PATH?.trim();

  if (!publicKey) {
    throw new Error('Для режима yandex-public нужно указать YANDEX_DISK_PUBLIC_KEY.');
  }
  ...
  const href = await fetchDownloadUrl(`${PUBLIC_DOWNLOAD_ENDPOINT}?${params.toString()}`);
  const buffer = await fetchFileBuffer(href);
  ...
  return {
    ...
    fileName: publicPath ? basenameFromFilePath(publicPath) : 'public-journal.xlsx',
    groupNameHint: publicPath ? filenameToGroupName(basenameFromFilePath(publicPath)) : 'Журнал',
  };
}
