...
function parseReportCards(workbook: XLSX.WorkBook, students: StudentRecord[]): ReportCard[] {
  ...
  for (let row = 1; row <= range.e.r + 1; row += 1) {
    const studentName = getCellText(sheet, row, 1);
    if (!studentName) {
      continue;
    }
  ...
  for (let index = 0; index < blockStarts.length; index += 1) {
    ...
    const studentName = getCellText(sheet, startRow, 1) || `Студент ${cards.length + 1}`;
    ...
    for (let row = startRow + 3; row <= subjectsEndRow; row += 1) {
      ...
      rows.push({
        ...
        subjectName,
        session: getCellText(sheet, row, 3) || null,
        average: parseNumber(getCellText(sheet, row, 4)),
        absences: {
          valid: parseNumber(getCellText(sheet, row, 5)) ?? 0,
          invalid: parseNumber(getCellText(sheet, row, 6)) ?? 0,
        },
      });
    }

    if (summaryRow && summaryRow >= startRow + 3) {
      const summaryAverage = parseNumber(getCellText(sheet, summaryRow, 4));
      const summaryValid = parseNumber(getCellText(sheet, summaryRow, 5));
      const summaryInvalid = parseNumber(getCellText(sheet, summaryRow, 6));
    ...
    if (totalMarkerRow && totalMarkerRow + 1 <= endRow) {
      const totalValue = parseNumber(getCellText(sheet, totalMarkerRow + 1, 5));
      if (totalValue !== null) {
